...
# There are three DEMs dataset availabe but in different spatial resolution (i.e. 1m, 5m & 25m). 
# This scritp deals with the 25m DEMs and return the longitudinal profile for them, respectively.
#########################
...
longitudinal.profile.value<-function(longprofile=longprofile.1m,flowdir=flowdir.1m){
  ...
  value.row[value.col==0]<-value.row[value.col==0]-1
  value.col[value.col==0]<-raster::ncol(longprofile)
...
value.25m<-longitudinal.profile.value(longprofile = longprofile.25m,flowdir = flowdir.25m)
